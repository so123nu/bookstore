    ...
    <div class="row">
      <div class="col-md-3">
        ...
        <button
          ...
          class="btn btn-primary btn-lg mt-3 form-control"
        >
          Buy
        </button>
      </div>
        ...
        <div class="modal-body" style="padding: 0 !important">
          <div id="Checkout" class="inline">
            ...
            <form @submit.prevent="purchaseBook">
              ...
              <button
                id="PayButton"
                class="btn btn-block btn-success submit-button form-control"
                type="submit"
                v-if="!paymentForm.isLoading"
              >
                <span class="submit-button-lock"></span>
                <span class="align-middle">Pay &#x20b9; {{ book.price }}</span>
              </button>
              <button
                id="PayButton"
                class="btn btn-block btn-success submit-button form-control"
              ...
              >
                <span class="submit-button-lock"></span>
                <span class="align-middle">Processing Payment</span>
              </button>
            </form>
          </div>
        </div>
...
export default {
  setup() {
    ...
    function resetForm() {
      Object.assign(paymentForm, initialState);
    }
    ...
    async function purchaseBook() {
      ...
      await axios
        ...
        .then(function (response) {
          if (response.data.statuscode == 200) {
            createToast("Payment Successful.!", { type: "success" });
    ...
    function separateYear() {
      let arr = paymentForm.expiry.split("/");
      paymentForm.exp_month = arr[0];
      paymentForm.exp_year = arr[1];
    }
